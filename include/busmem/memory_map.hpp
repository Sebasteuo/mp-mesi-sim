...
#include <cstdint>
#include <stdexcept>
#include "busmem/messagesTypes.hpp"  

inline constexpr std::uint64_t align_to(std::uint64_t x, std::uint64_t a) {
    return (x + (a - 1)) & ~(a - 1);
}
...
struct MemLayout {
    ...
    std::uint64_t SIZE_A        = 0;   // N * 8
    std::uint64_t SIZE_B        = 0;   // N * 8
    std::uint64_t SIZE_PARTIALS = 0;   // PES * (8 o LINE_SIZE)
    ...
    void compute(std::uint32_t n, std::uint32_t hw_threads, std::uint32_t pes,
                 bool align_partials_to_line)
    {
        ...
        SIZE_PARTIALS = align_partials_to_line ? std::uint64_t(pes) * LINE_SIZE
                                               : std::uint64_t(pes) * 8;
